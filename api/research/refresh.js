...
export default async function handler(req, res) {
  ...
  try {
    ...
    const rows = keys.map((label,i) => {
      ...
      return { region, week_of: week, label, heat, momentum, forecast_heat, confidence, act_watch_avoid: awa, links: [] };
    });
    ...
    const data = rows.sort((a,b)=> b.heat - a.heat).map(r => ({
      theme: r.label, heat: r.heat, momentum: r.momentum, forecast_heat: r.forecast_heat,
      confidence: r.confidence, act_watch_avoid: r.act_watch_avoid, links: r.links
    }));
